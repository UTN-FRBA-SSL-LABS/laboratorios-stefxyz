...
> R:Inicializa el puntero en 0,es importante ya que de no hacerlo podria tener basura ya guardada y al agregar un valor lo agregaras sobre la basura
...
> R:falla,esperaba 700 y dio 350
...
> R:El bug esta en la linea 24 y la linea dice "total += c->items[i].precio;"

**P7** — ¿Qué debería hacer esa línea para calcular el total correctamente?

> R:Se deberia multiplicar por la cantidad
...
> R:Falla,ese ultimo carrito_agregar() devuelve 1
...
> R:El operador incorrecto es <=,deberia ser solo <
...
> R:Las lineas 36 y 38 tienen ####,esto debido a que no se unan en test_unitarios.c y este es lo unico que se cubre con el gcov como se ve en la linea 25 del makefile
...
> R:La diferencia es que los unitarios son mas compactos, especificos y porecisos, mientras que los de integracion lo contrario ya que se prueba como funcionan varios modulos juntos, ese bug lo detecto primero el unitario ya que en uno de los test se buscaba ver si funcionaba cuando se ponia x cantidad pero se vio que no
...
> R:Esto es peligroso es C ya que no genera un fallo que haga que el programa explote y no se puede compilar seno que lo permite pero por detras corrompio los datos sin saberlo de no estar este check,mientras que en java si te avisa si hubo un mal manejo de memoria lo cual hace que no se compile
...
> R:Una de las mejoras es que es mas facil de buscarlo pues para eso uso los  ASSERT_IGUAL a diferencia de un pritf que puede que lo use para eso o mensaje del codigo lo cual me haria perder un poco de tiempo, ademas muestra exactamente que es lo que deberia dary cuanto me dio realmente 
...
> R:No lo detecto en el primero porque al tener canttidad=1 da el mismo resultado que si no multiplicamos por nada
